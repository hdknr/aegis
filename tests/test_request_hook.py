...
class TestRequestHookDomainWhitelist:
    def test_html_from_any_domain_allowed(self, addon):
        flow = make_flow("https://unknown.com/page.html", "unknown.com")
        addon.request(flow)
        assert flow.response is None

    def test_binary_from_whitelisted_domain_allowed(self, addon):
        flow = make_flow("https://github.com/release.tar.gz", "github.com")
        addon.request(flow)
        assert flow.response is None

    def test_binary_from_unknown_domain_blocked(self, addon):
        flow = make_flow("https://evil.com/malware.tar.gz", "evil.com")
        addon.request(flow)
    ...
    def test_exe_from_unknown_domain_blocked(self, addon):
        flow = make_flow("https://evil.com/setup.exe", "evil.com")
        addon.request(flow)
        assert flow.response is not None
        assert flow.response.status_code == 403
...
class TestRequestHookC2Blocklist:
    def test_c2_ip_blocked(self, addon):
        flow = make_flow("https://c2server.com/callback", "c2server.com")
    ...
    def test_safe_ip_allowed(self, addon):
        flow = make_flow("https://safe.com/page.html", "safe.com")
    ...
    def test_dns_failure_allowed(self, addon):
        ...
        flow = make_flow("https://unknown.com/page", "unknown.com")
        import socket
